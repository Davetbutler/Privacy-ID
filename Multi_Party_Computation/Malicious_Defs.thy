...
definition ideal_view_1 :: "'in1 \<Rightarrow> 'in2 \<Rightarrow> 'aux \<Rightarrow> 'P1_real_adv \<Rightarrow> ('adv_out1 \<times> 'out2) spmf"
  where "ideal_view_1 x y z \<A> = (ideal_game_1 x y z (S1_1 \<A>, S1_2 \<A>))" 

text\<open>We have information theoretic security when the real and ideal views are equal.\<close>

definition "perfect_sec_P1 x y z \<A> \<longleftrightarrow> (ideal_view_1 x y z \<A> = P1_real_view x y z \<A>)"

text\<open>The advantage of party 1 denotes the probability of a distinguisher distinguishing the real and 
ideal views.\<close>

definition "adv_P1 x y z \<A> (D :: ('adv_out1 \<times> 'out2) \<Rightarrow> bool spmf) = 
                \<bar>spmf (P1_real_view x y z \<A> \<bind> (\<lambda> view. D view)) True
                    - spmf (ideal_view_1 x y z \<A> \<bind> (\<lambda> view. D view)) True \<bar>" 
...
definition ideal_view_2 :: "'in1 \<Rightarrow> 'in2 \<Rightarrow> 'aux \<Rightarrow> 'P2_real_adv \<Rightarrow> ('out1 \<times> 'adv_out2) spmf"
  where "ideal_view_2 x y z \<A> = (ideal_game_2 x y z (S2_1 \<A>, S2_2 \<A>))" 

definition "perfect_sec_P2 x y z \<A> \<longleftrightarrow> (ideal_view_2 x y z \<A> = P2_real_view x y z \<A>)"

definition "adv_P2 x y z \<A> (D :: ('out1 \<times> 'adv_out2) \<Rightarrow> bool spmf) = 
                \<bar>spmf (P2_real_view x y z \<A> \<bind> (\<lambda> view. D view)) True
                    - spmf (ideal_view_2 x y z \<A> \<bind> (\<lambda> view. D view)) True \<bar>" 
