...
fun valid_inputs_np_ot12 where "valid_inputs_np_ot12 [M (m0,m1), C \<sigma>] = (m0 \<in> carrier \<G> \<and> m1 \<in> carrier \<G>)"
...
fun protocol :: "'grp inputs_ot12 list \<Rightarrow> ('grp outputs_ot12 list) spmf"
  where "protocol [M (m0,m1), C \<sigma>] = do {
    ...
    return_spmf [U (), P out_2]}"

lemma lossless_protocol: "lossless_spmf (protocol [M (m0,m1), C \<sigma>])"
...
type_synonym 'grp' view1 = "('grp' inputs_ot12 \<times> ('grp' \<times> 'grp' \<times> 'grp' \<times> 'grp')) spmf"
...
fun R1 :: "'grp inputs_ot12 list \<Rightarrow> 'grp view1"
  where "R1 [M (m0,m1), C \<sigma>] = do {
    a \<leftarrow> sample_uniform (order \<G>);
    b \<leftarrow> sample_uniform (order \<G>);
    let c\<^sub>\<sigma> = a*b;
    c\<^sub>\<sigma>' \<leftarrow> sample_uniform (order \<G>);
    return_spmf (M (m0,m1), (\<^bold>g [^] a, \<^bold>g [^] b, (if \<sigma> then \<^bold>g [^] c\<^sub>\<sigma>' else \<^bold>g [^] c\<^sub>\<sigma>), (if \<sigma> then \<^bold>g [^] c\<^sub>\<sigma> else \<^bold>g [^] c\<^sub>\<sigma>')))}"  

lemma lossless_R1: "lossless_spmf (R1 [M (m0,m1), C \<sigma>])"
  by(simp add: Let_def lossless_sample_uniform_units or_gt_0)

fun inter :: "'grp inputs_ot12 \<Rightarrow> 'grp view1"
  where "inter (M (m0,m1)) = do {
    a \<leftarrow> sample_uniform (order \<G>);
    b \<leftarrow> sample_uniform (order \<G>);  
    c \<leftarrow> sample_uniform (order \<G>);
    d \<leftarrow> sample_uniform (order \<G>);
    return_spmf (M (m0,m1), \<^bold>g [^] a, \<^bold>g [^] b, \<^bold>g [^] c, \<^bold>g [^] d)}"

fun S1 :: "'grp inputs_ot12 \<Rightarrow> 'grp outputs_ot12 \<Rightarrow> 'grp view1"
  where "S1 (M (m0,m1)) (U ()) = do {
    a \<leftarrow> sample_uniform (order \<G>);
    b \<leftarrow> sample_uniform (order \<G>);
    c \<leftarrow> sample_uniform (order \<G>);
    return_spmf (M (m0,m1), (\<^bold>g [^] a, \<^bold>g [^] b, \<^bold>g [^] c, \<^bold>g [^] (a*b)))}"  

lemma lossless_S1: "lossless_spmf (S1 (M (m0,m1)) (U ()))"
  by(simp add: Let_def lossless_sample_uniform_units or_gt_0)
...
fun R2 :: " 'grp inputs_ot12 list \<Rightarrow> ('grp inputs_ot12 \<times> 'grp \<times> 'grp \<times>  'grp \<times> 'grp \<times> 'grp \<times> 'grp \<times> 'grp) spmf" 
  where "R2 [M (m0,m1), C \<sigma>]  = do {
  a :: nat \<leftarrow> sample_uniform (order \<G>);
  b :: nat \<leftarrow> sample_uniform (order \<G>);
  let c\<^sub>v = (a*b) mod (order \<G>);
  c\<^sub>v' :: nat \<leftarrow> sample_uniform (order \<G>);
  ...
  return_spmf(C \<sigma>, \<^bold>g [^] a, \<^bold>g [^] b, \<^bold>g [^] c\<^sub>v, w0, enc_m, w1, enc_m')}" 

lemma lossless_R2: "lossless_spmf (R2 [M (m0,m1), C \<sigma>])"
  apply(simp add: Let_def split_def lossless_sample_uniform_units or_gt_0)
  using prime_order prime_gt_1_nat lossless_sample_uniform_units by simp

fun S2 :: "'grp inputs_ot12 \<Rightarrow> 'grp outputs_ot12 \<Rightarrow> ('grp inputs_ot12 \<times> 'grp \<times> 'grp \<times> 'grp \<times> 'grp \<times> 'grp \<times> 'grp \<times> 'grp) spmf" 
  where "S2 (C \<sigma>) (P m) =  do {
  a :: nat \<leftarrow> sample_uniform (order \<G>);
  b :: nat \<leftarrow> sample_uniform (order \<G>);
  let c\<^sub>v = (a*b) mod (order \<G>);
  r0 :: nat \<leftarrow> sample_uniform_units (order \<G>);
...
lemma lossless_S2: "lossless_spmf (S2 (C \<sigma>) (P m))"
...
sublocale np_P1: semi_honest_det_security protocol f_ot12 valid_inputs_np_ot12 0 R1 S1 .

sublocale np_P2: semi_honest_det_security protocol f_ot12 valid_inputs_np_ot12 1 R2 S2 .
...
lemma correctness: 
  assumes "m0 \<in> carrier \<G>" "m1 \<in> carrier \<G>" 
  shows  "protocol [M (m0,m1), C \<sigma>] = f_ot12 [M (m0,m1), C \<sigma>]"
proof-
  have "protocol [M (m0,m1), C \<sigma>] = do {
    a :: nat \<leftarrow> sample_uniform (order \<G>);
    b :: nat \<leftarrow> sample_uniform (order \<G>);
    r1 :: nat \<leftarrow> sample_uniform_units (order \<G>);
    s1 :: nat \<leftarrow> sample_uniform_units (order \<G>);
...
theorem correct_np: "np_correct.correctness [M (m0,m1), C \<sigma>]"
  unfolding np_correct.correctness_def using correctness by simp 

lemma security_P1: 
  shows "np_P1.advantage [M (m0,m1), C \<sigma>] D \<le> ddh.advantage (R1_inter_adversary D (M (m0,m1))) + ddh.advantage (inter_S1_adversary D (M (m0,m1)))"
    (is "?lhs \<le> ?rhs")
proof(cases \<sigma>)
  case True
  have "R1 [M (m0,m1), C \<sigma>] = S1 (M (m0,m1)) (U ())" for out1
    by(simp add: True)
  then have "np_P1.advantage [M (m0,m1), C \<sigma>] D = 0"
    by(simp add: np_P1.advantage_def) 
...
next
  ...
  also have R1_inter_dist: "\<bar>spmf (R1 [M (m0,m1), C False] \<bind> D) True - spmf ((inter (M (m0,m1))) \<bind> D) True\<bar> = ddh.advantage (R1_inter_adversary D (M (m0,m1)))"
    unfolding ddh.advantage_def ddh.ddh_0_def ddh.ddh_1_def Let_def split_def by simp
  also  have inter_S1_dist: "\<bar>spmf ((inter (M (m0,m1))) \<bind> D) True - spmf (S1 (M (m0,m1)) (U ()) \<bind> D) True\<bar> = ddh.advantage (inter_S1_adversary D (M (m0,m1)))" 
    including monad_normalisation
    by(simp add: ddh.advantage_def ddh.ddh_0_def ddh.ddh_1_def) 
  ultimately have "\<bar>spmf (R1 [M (m0,m1), C False] \<bind> (\<lambda>view. D view)) True - spmf (S1 (M (m0,m1)) (U ()) \<bind> (\<lambda>view. D view)) True\<bar> \<le> ?rhs"
...
lemma security_P2: 
  assumes "m0 \<in> carrier \<G>" "m1 \<in> carrier \<G>"
  shows "R2 [M (m0,m1), C \<sigma>] = S2 (C \<sigma>) (if \<sigma> then (P m1) else (P m0))"
  including monad_normalisation
proof-
  have "R2 [M (m0,m1), C \<sigma>] = do {
      a :: nat \<leftarrow> sample_uniform (order \<G>);
      b :: nat \<leftarrow> sample_uniform (order \<G>);
      let c\<^sub>v = (a*b) mod (order \<G>);
      c\<^sub>v' :: nat \<leftarrow> sample_uniform (order \<G>);
...
theorem np_perfect_security_P2:
  shows "np_P2.perfect_security [M (m0,m1), C \<sigma>]"
  unfolding np_P2.perfect_security_def using security_P2 by auto
...
theorem correctness_asymp: 
  shows "np_correct.correctness \<eta> [M (m0,m1), C \<sigma>]"
  by(rule correct_np) 

theorem security_P1_asymp: 
  assumes "negligible (\<lambda> \<eta>. ddh.advantage \<eta> (inter_S1_adversary \<eta> D (M (m0,m1))))"
    and "negligible (\<lambda> \<eta>. ddh.advantage \<eta> (R1_inter_adversary \<eta>  D (M (m0,m1))))"
  shows "negligible (\<lambda> \<eta>. np_P1.advantage \<eta> [M (m0,m1), C \<sigma>] D)"
proof-
  have "np_P1.advantage \<eta> [M (m0,m1), C \<sigma>] D \<le> ddh.advantage \<eta> (R1_inter_adversary \<eta>  D (M (m0,m1))) + ddh.advantage \<eta> (inter_S1_adversary \<eta> D (M (m0,m1)))" 
    for \<eta>
    using security_P1 by simp
  moreover have "negligible (\<lambda> \<eta>. ddh.advantage \<eta> (R1_inter_adversary \<eta>  D (M (m0,m1))) + ddh.advantage \<eta> (inter_S1_adversary \<eta> D (M (m0,m1))))"
    using assms 
...
theorem np_perfect_security_asymp: 
  shows "np_P2.perfect_security \<eta> [M (m0,m1), C \<sigma>]"
