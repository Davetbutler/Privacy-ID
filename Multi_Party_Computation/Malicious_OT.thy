...
lemma P1_ideal_ideal_eq:
  shows  "mal_def.ideal_view_1 x y z \<A> = P1_ideal_model x y z \<A>"
...
lemma P1_advantages_eq: 
  shows "mal_def.adv_P1 x y z \<A> D = P1_adv_real_ideal_model D x y \<A> z"
...
lemma P2_ideal_view_unfold: "mal_def.ideal_view_2 (x0,x1) \<sigma> z \<A> = P2_ideal_model (x0,x1) \<sigma> z \<A>"
...
lemma P1_real_ideal_DDH_advantage_false:
  assumes "\<sigma> = False" 
  shows "mal_def.adv_P1 M \<sigma> z \<A> D = ddh.DDH_advantage (P1_DDH_mal_adv_\<sigma>_false M z \<A> D)"
...
lemma P1_real_ideal_DDH_advantage_false_bound:
  assumes "\<sigma> = False"
  shows "mal_def.adv_P1 M \<sigma> z \<A> D 
...
lemma P1_real_ideal_DDH_advantage_true:
  assumes "\<sigma> = True" 
  shows "mal_def.adv_P1 M \<sigma> z \<A> D = ddh.DDH_advantage (P1_DDH_mal_adv_\<sigma>_true M z \<A> D)"
...
proof-
  ...
  proof-
    ...
    qed 
    then show ?thesis using finite_carrier pow_generator_eq_iff_cong by blast
  qed
  ...
  proof-
    ...
    proof-
      ...
      qed
      thus ?thesis using finite_carrier pow_generator_eq_iff_cong by blast
    qed
...
proof-
  ...
  proof-
    ...
    thus ?thesis by simp
  qed
  hence "\<^bold>g [^] (t * (u0 + (s * (nat ((fst (bezw t (order \<G>))) mod order \<G>))))) = \<^bold>g [^] (t * u0 + s)" using finite_carrier pow_generator_eq_iff_cong by blast
  thus ?thesis 
...
lemma malicious_sec_P2:   
  assumes x1_in_carrier: "x1 \<in> carrier \<G>"
    and x0_in_carrier: "x0 \<in> carrier \<G>"  
  shows "mal_def.perfect_sec_P2 (x0,x1) \<sigma> z \<A>"
...
lemma P1_security_asym:
  "negligible (\<lambda> n. mal_def.adv_P1 n M \<sigma> z \<A> D)" 
...
proof-
  ...
  proof(cases \<sigma>)
    case True
    have bound_mod: "\<bar>mal_def.adv_P1 n M \<sigma> z  \<A> D\<bar> 
...
lemma P2_security_asym:   
  ...
  shows "mal_def.perfect_sec_P2 n (x0,x1) \<sigma> z \<A>"
