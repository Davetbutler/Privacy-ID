...
fun f_ot12 :: "inputs_ot12 list \<Rightarrow> (outputs_ot list) spmf"
...
fun f_ot14 :: "inputs_ot14 list \<Rightarrow> (outputs_ot list) spmf"
...
lemma ot14_correct_unfold: "\<forall> c0 c1 m00 m01 m10 m11. protocol_ot14 [M4 (m00, m01, m10, m11), C2 (c0,c1)] =  f_ot14 [M4 (m00, m01, m10, m11), C2 (c0,c1)]"
  using ot14_correct unfolding ot14.correctness_def by auto

lemma inf_th_14_OT_P4: "R2_OT14 [M4 (m00, m01, m10, m11), C2 (c0,c1)] = (f_ot14 [M4 (m00, m01, m10, m11), C2 (c0,c1)] \<bind> (\<lambda> outputs_ot. S2_OT14 (C2 (c0,c1)) (nth outputs_ot 1)))" 
  using ot14_perfect_security_P2[unfolded ot14_2.perfect_security_def] by auto

lemma ass_adv_14_OT: "\<bar>spmf (bind_spmf (S1_OT14 (M4 (m00, m01, m10, m11)) (U())) (\<lambda> view. (D view))) True - 
                    spmf (bind_spmf (R1_OT14 [M4 (m00, m01, m10, m11), C2 (c0,c1)]) (\<lambda> view. (D view))) True \<bar> \<le> ot14_adv_P1"
proof-
  have "f_ot14 [M4 (m00, m01, m10, m11), C2 (c0,c1)] \<bind> (\<lambda> outputs_ot. S1_OT14 (M4 (m00, m01, m10, m11)) (nth outputs_ot 0) \<bind> D) = S1_OT14 (M4 (m00, m01, m10, m11)) (U()) \<bind> D" 
    by auto
  thus ?thesis using ot14_advantage_P1[unfolded ot14_1.advantage_def]
...
fun xor_funct :: "share_wire list \<Rightarrow> bool list spmf"
...
definition "randomness = coin_spmf" 

fun  and_R1 :: "gmw_inputs list \<Rightarrow> bool \<Rightarrow> ((gmw_inputs \<times> bool \<times> 'ot14_view1)) spmf"
  where "and_R1 [A, B] \<sigma> = do {
    let (a1, a2) = A;
    let (b1,b2) = B;
...
fun and_outputs :: "gmw_inputs list \<Rightarrow> bool \<Rightarrow> (gmw_outputs list) spmf"
  where "and_outputs [A, B] \<sigma> = do {
            let (a1, a2) = A;
            let (b1,b2) = B;
            let s0 = \<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False));   
            let s1 = \<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True));   
...
fun S1_and :: "gmw_inputs \<Rightarrow> gmw_outputs \<Rightarrow> ((gmw_inputs \<times> bool \<times> 'ot14_view1)) spmf"
...
sublocale gmw_and_1: semi_honest_prob 0 and_funct and_protocol and_outputs randomness and_R1 S1_and valid_inputs_and .

term "\<bar>spmf ((gmw_and_1.real_view [(a1,a2),(b1,b2)]) \<bind> D) True - spmf ((gmw_and_1.ideal_view [(a1,a2),(b1,b2)] 
            \<bind> (D :: (((bool \<times> bool) \<times> bool \<times> 'ot14_view1) \<times> gmw_outputs list) \<Rightarrow> bool spmf))) True\<bar>"

lemma sec_ex_P1_and: 
  shows "\<exists> (A :: 'ot14_view1 \<Rightarrow> bool \<Rightarrow> bool spmf).
          \<bar>spmf ((gmw_and_1.real_view [(a1,a2),(b1,b2)]) \<bind> D) True - spmf ((gmw_and_1.ideal_view [(a1,a2),(b1,b2)] 
            \<bind> (D :: (((bool \<times> bool) \<times> bool \<times> 'ot14_view1) \<times> gmw_outputs list) \<Rightarrow> bool spmf))) True\<bar> =
              \<bar>spmf (coin_spmf \<bind> (\<lambda> \<sigma>. R1_OT14 [(M4 ((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True))))), (C2 (b1, b2))] 
                    \<bind> (\<lambda> view. A view \<sigma>))) True - spmf (coin_spmf \<bind> (\<lambda> \<sigma>. S1_OT14 (M4((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True))))) (U ()) 
                \<bind>  (\<lambda> view. A view \<sigma>))) True \<bar>"
  including monad_normalisation
proof-
  define A' where "A' == \<lambda> view \<sigma>.  (D (((a1,a2), \<sigma>, view), [Q \<sigma>, Q (\<sigma> \<oplus> ((a1 \<oplus> b1) \<and> (a2 \<oplus> b2)))]))" 
  have "\<bar>spmf ((gmw_and_1.real_view [(a1,a2),(b1,b2)]) \<bind> (D)) True - spmf ((gmw_and_1.ideal_view [(a1,a2),(b1,b2)]  
          \<bind> (D))) True\<bar> =
              \<bar>spmf (coin_spmf \<bind> (\<lambda> \<sigma>. R1_OT14 [M4((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True)))), C2 (b1, b2)] 
                  \<bind> (\<lambda> view. A' view \<sigma>))) True - spmf (coin_spmf \<bind> (\<lambda> \<sigma>. S1_OT14 ((M4 ((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True)))))) (U ()) 
                \<bind>  (\<lambda> view. A' view \<sigma>))) True\<bar>"
    apply(auto simp add:  semi_honest_prob.ideal_view_def gmw_and_1.real_view_def  A'_def   Let_def split_def ot14_correct_unfold ; cases a1; cases a2; cases b1; cases b2; auto)
    unfolding randomness_def by auto
...
lemma bound_14_OT:
  "\<bar>spmf (coin_spmf \<bind> (\<lambda> \<sigma>. R1_OT14 [M4 ((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True)))), C2 (b1, b2)] 
              \<bind> (\<lambda> view. A view \<sigma>))) True - spmf (coin_spmf \<bind> (\<lambda> \<sigma>. S1_OT14 (M4 ((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True))))) (U ()) 
        \<bind>  (\<lambda> view. (A :: 'ot14_view1 \<Rightarrow> bool \<Rightarrow> bool spmf) view \<sigma>))) True\<bar> \<le> ot14_adv_P1"
  (is "?lhs \<le> ot14_adv_P1")
proof-
  have int1: "integrable (measure_spmf coin_spmf) (\<lambda>x. spmf (S1_OT14 (M4 (x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> True), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> True))) (U ()) \<bind> (\<lambda>view. A view x)) True)"
    and int2: "integrable (measure_spmf coin_spmf) (\<lambda>x. spmf (R1_OT14 [M4 (x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> True), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> True)), C2 (b1, b2)] \<bind> (\<lambda>view. A view x)) True)"
    by(rule measure_spmf.integrable_const_bound[where B=1]; simp add: pmf_le_1)+
  have "?lhs = \<bar>LINT x|measure_spmf coin_spmf.
        spmf (S1_OT14 (M4 (x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> True), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> True))) (U ()) \<bind> (\<lambda>view. A view x)) True -
        spmf (R1_OT14 [(M4 (x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> False \<and> a2 \<oplus> True), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> False), x \<oplus> (a1 \<oplus> True \<and> a2 \<oplus> True))), C2 (b1, b2)] \<bind> (\<lambda>view. A view x)) True\<bar>"
    apply(subst (1 2) spmf_bind) using int1 int2 by simp
  also have "... \<le> LINT x|measure_spmf coin_spmf. \<bar>spmf (S1_OT14 (M4 (x = (a1 \<longrightarrow> \<not> a2), x = (a1 \<longrightarrow> a2), x = (a1 \<or> \<not> a2), x = (a1 \<or> a2))) (U ()) \<bind> (\<lambda>view. A view x)) True 
                - spmf (R1_OT14 [M4 (x = (a1 \<longrightarrow> \<not> a2), x = (a1 \<longrightarrow> a2), x = (a1 \<or> \<not> a2), x = (a1 \<or> a2)), C2 (b1, b2)] \<bind> (\<lambda>view. A view x)) True\<bar>"
    by(rule integral_abs_bound[THEN order_trans]; simp add: split_beta)
  ultimately have "?lhs \<le> LINT x|measure_spmf coin_spmf. \<bar>spmf (S1_OT14 (M4 (x = (a1 \<longrightarrow> \<not> a2), x = (a1 \<longrightarrow> a2), x = (a1 \<or> \<not> a2), x = (a1 \<or> a2))) (U ()) \<bind> (\<lambda>view. A view x)) True 
                - spmf (R1_OT14 [M4 (x = (a1 \<longrightarrow> \<not> a2), x = (a1 \<longrightarrow> a2), x = (a1 \<or> \<not> a2), x = (a1 \<or> a2)), C2 (b1, b2)] \<bind> (\<lambda>view. A view x)) True\<bar>"
    by simp
  also have "LINT x|measure_spmf coin_spmf. \<bar>spmf (S1_OT14 (M4 (x = (a1 \<longrightarrow> \<not> a2), x = (a1 \<longrightarrow> a2), x = (a1 \<or> \<not> a2), x = (a1 \<or> a2))) (U ()) \<bind> (\<lambda>view. A view x)) True 
                - spmf (R1_OT14 [M4 (x = (a1 \<longrightarrow> \<not> a2), x = (a1 \<longrightarrow> a2), x = (a1 \<or> \<not> a2), x = (a1 \<or> a2)), C2 (b1, b2)] \<bind> (\<lambda>view. A view x)) True\<bar> \<le> ot14_adv_P1"
    ...
    subgoal for M
      using pmf_le_1[of "R1_OT14 [M4 (\<not> M, M, M, M), C2 (b1,b2)] \<bind>  (\<lambda> view. A view M)" "Some True"]
        pmf_le_1[of "S1_OT14 (M4 (\<not> M, M, M, M)) (U ()) \<bind>  (\<lambda> view. A view M)" "Some True"] 
      by simp
    subgoal for M 
      using pmf_le_1[of "R1_OT14 [M4 (M, \<not> M, M, M), C2 (b1,b2)] \<bind>  (\<lambda> view. A view M)" "Some True"] 
        pmf_le_1[of "S1_OT14 (M4 (M, \<not> M, M, M)) (U ()) \<bind>  (\<lambda> view. A view M)" "Some True"] 
      by simp
        apply(cases a2) apply(auto)
    subgoal for M
      using pmf_le_1[of "R1_OT14 [M4 (M, M, \<not> M, M), C2 (b1,b2)] \<bind>  (\<lambda> view. A view M)" "Some True"] 
        pmf_le_1[of "S1_OT14 (M4 (M, M, \<not> M, M)) (U ()) \<bind>  (\<lambda> view. A view M)" "Some True"] 
      by(simp)
    subgoal for M
      using pmf_le_1[of "R1_OT14 [M4 (M, M, M, \<not> M), C2 (b1,b2)] \<bind>  (\<lambda> view. A view M)" "Some True"] 
        pmf_le_1[of "S1_OT14 (M4 (M, M, M, \<not> M)) (U ()) \<bind>  (\<lambda> view. A view M)" "Some True"] 
      by(simp)
...
proof-
  obtain A :: "'ot14_view1 \<Rightarrow> bool \<Rightarrow> bool spmf" where "\<bar>spmf ((gmw_and_1.real_view [(a1,a2),(b1,b2)]) \<bind> D) True - spmf ((gmw_and_1.ideal_view [(a1,a2),(b1,b2)] 
            \<bind> (D :: (((bool \<times> bool) \<times> bool \<times> 'ot14_view1) \<times> gmw_outputs list) \<Rightarrow> bool spmf))) True\<bar> =
              \<bar>spmf (coin_spmf \<bind> (\<lambda> \<sigma>. R1_OT14 [(M4 ((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True))))), (C2 (b1, b2))] 
                    \<bind> (\<lambda> view. A view \<sigma>))) True - spmf (coin_spmf \<bind> (\<lambda> \<sigma>. S1_OT14 (M4((\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False))), (\<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True))))) (U ()) 
                \<bind>  (\<lambda> view. A view \<sigma>))) True\<bar>"
...
fun and_R2 :: "gmw_inputs list \<Rightarrow> bool \<Rightarrow> (((bool \<times> bool) \<times> 'ot14_view2)) spmf"
  where "and_R2 [A, B] \<sigma> = do {
    let (a1, a2) = A;
    let (b1,b2) = B;
    let s0 = \<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> False));   
    let s1 = \<sigma> \<oplus> ((a1 \<oplus> False) \<and> (a2 \<oplus> True));   
    let s2 = \<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> False));   
    let s3 = \<sigma> \<oplus> ((a1 \<oplus> True) \<and> (a2 \<oplus> True)); 
    V :: 'ot14_view2 \<leftarrow> R2_OT14 [M4 (s0,s1,s2,s3), C2 B];
    return_spmf ((B, V))}"

fun S2_and :: "gmw_inputs \<Rightarrow> gmw_outputs \<Rightarrow> (((bool \<times> bool) \<times> 'ot14_view2)) spmf"
  where "S2_and B (Q s2) =  do {
    let (a2,b2) = B;
    V :: 'ot14_view2 \<leftarrow> S2_OT14 (C2 (a2,b2)) (P s2);
    return_spmf ((B, V))}"

sublocale gmw_and_2: semi_honest_prob 1 and_funct and_protocol and_outputs randomness and_R2 S2_and valid_inputs_and .

lemma and_perfect_security_P2: 
  shows "gmw_and_2.perfect_security [(a1,a2),(b1,b2)]"
  unfolding gmw_and_2.perfect_security_def semi_honest_prob.ideal_view_def gmw_and_2.real_view_def 
  apply(simp add: randomness_def ot14_correct_unfold inf_th_14_OT_P4)
