...
locale semi_honest_prob = 
  fixes i :: nat
    ...
    and protocol :: "'input list \<Rightarrow> ('output list) spmf"
    and outputs :: "'input list \<Rightarrow> ('rand \<times> 'view) \<Rightarrow> ('output list) spmf"
    and randomness :: "'rand spmf" 
      \<comment> \<open>a list of all randomness created in the protocol, we will give the correct parts to the respective parties when defining the views\<close>
    and real_view_msgs :: "'input list \<Rightarrow> 'rand \<Rightarrow> ('rand \<times> 'view) spmf" \<comment> \<open>the real view of the ith party\<close>
    and sim :: "'input \<Rightarrow> 'output \<Rightarrow> ('rand \<times> 'view) spmf"
    and valid_inputs :: "'input list \<Rightarrow> bool"
begin

definition real_view :: "'input list \<Rightarrow> (('rand \<times> 'view) \<times> 'output list) spmf"
...
definition ideal_view :: "'input list \<Rightarrow> (('rand \<times> 'view) \<times> 'output list) spmf"
  where "ideal_view inputs = do {
   outputs :: 'output list \<leftarrow> funct inputs;
   view  \<leftarrow> sim (nth inputs i) (nth outputs i);
   return_spmf (view, outputs)}"

definition "perfect_security inputs \<equiv> (valid_inputs inputs \<longrightarrow> real_view inputs = ideal_view inputs)"

definition advantage :: "'input list \<Rightarrow> ((('rand \<times> 'view) \<times> 'output list) \<Rightarrow> bool spmf) \<Rightarrow> real"
